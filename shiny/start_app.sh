...
# FPsim Family Planning Simulation Shiny App - Auto Install and Start Script
# This script automatically installs all dependencies and starts the app
# Run from the shiny/ directory

set -e  # Exit on any error

echo "🚀 Starting FPsim Family Planning Shiny App Setup..."
echo "=============================================="
...
# Kill any existing R processes on port 3031
print_status "Checking for existing processes on port 3031..."
if lsof -i :3031 >/dev/null 2>&1; then
    print_warning "Found existing process on port 3031, killing it..."
...
# Install FPsim package in editable mode
print_status "Installing FPsim package..."
pip install -e .
...
print_status "Final cleanup - ensuring port 3031 is free..."
if lsof -i :3031 >/dev/null 2>&1; then
    lsof -ti :3031 | xargs kill -9 2>/dev/null || true
    sleep 2
fi
...
# Start the app
print_status "Starting the FPsim Family Planning Shiny App..."
print_success "Setup complete! Starting app on http://localhost:3031"
