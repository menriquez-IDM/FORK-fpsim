# FPsim Family Planning Shiny Web App - Complete Dependency Installer
# This script installs all required dependencies for the FPsim Shiny app

cat("=== FPsim Family Planning Shiny Web App - Dependency Installer ===\n\n")
...
cat("\n3. Installing FPsim package...\n")

# Install FPsim in editable mode
if (require(reticulate, quietly = TRUE)) {
  tryCatch({
    py_run_string("
import subprocess
...
if result.returncode == 0:
    print('✓ FPsim package installed successfully')
else:
    print('⚠ Error installing FPsim package')
    print(result.stderr)
")
  }, error = function(e) {
    cat("⚠ Error installing FPsim:", e$message, "\n")
  })
}
...
cat("The app will be available at: http://localhost:3031\n")
